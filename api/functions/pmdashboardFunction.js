    ...
    const completedTasksRows = resultsFromYourQuery.map((row) => {
      ...
      return {
        ...
        pending_percent:
          total > 0
            ? parseFloat(
                ((parseInt(row.pending_count, 10) / total) * 100).toFixed(2)
              )
            : 0,
        inprogress_percent:
          total > 0
            ? parseFloat(
                ((parseInt(row.inprogress_count, 10) / total) * 100).toFixed(2)
              )
            : 0,
        completed_percent:
          total > 0
            ? parseFloat(
                ((parseInt(row.completed_count, 10) / total) * 100).toFixed(2)
              )
            : 0,
      };
    });
