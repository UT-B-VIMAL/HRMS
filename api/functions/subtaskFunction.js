...
exports.createSubTask = async (payload, res, req) => {
  ...
  try {
    ...
    `;
    const values = [product_id, project_id, task_id, name, userId, userId];
...
exports.bulkimportSubTask = async (payload, res, req) => {
  ...
  try {
    ...
    const [[task]] = await db.query(
      `SELECT id FROM tasks WHERE user_id = ? AND product_id = ? AND project_id = ? AND name= ? AND deleted_at IS NULL`,
      [user_id, product_id, project_id, task_name]
    );
...
ORDER BY h.id DESC;
  ...
  `;
   ...
   ORDER BY c.updated_at DESC;
    ...
    if (Array.isArray(comments) && Array.isArray(comments[0])) {
      const grouped = new Map();

      for (const row of comments[0]) {
...
exports.deleteSubTask = async (req, res) => {
  ...
  try {
    ...
    const deleteQuery =
      "UPDATE sub_tasks SET deleted_at = NOW(), updated_by = ? WHERE id = ?";
