...
//   (SELECT CONVERT_TZ(start_time, '+00:00', 'Asia/Kolkata')  
//          FROM sub_tasks_user_timeline  
//          WHERE task_id = t.id  
//          ORDER BY end_time DESC  
//          LIMIT 1) AS start_time,  

//         -- Fetching latest end_time
//         (SELECT CONVERT_TZ(end_time, '+00:00', 'Asia/Kolkata')  
//          FROM sub_tasks_user_timeline  
//          WHERE task_id = t.id  
//          ORDER BY end_time DESC  
//          LIMIT 1) AS end_time,
...
exports.projectStatus = async (req, res) => {
  try {
    ...
    const formattedResults = results.map((row, index) => ({
      ...
      status: row.status === 0 ? "To Do" : row.status === 1 ? "In Progress" : "Done",
    ...
    successResponse(
      res,
      formattedResults,
      formattedResults.length === 0 ? "No data found" : "Data retrieved successfully",
      200,
      pagination
    );
  } catch (error) {
...
exports.projectRequest = async (req, res) => {
  try {
    ...
    const subtasksQuery = `
      ...
      LEFT JOIN tasks t ON t.id = st.task_id
      LEFT JOIN users u ON u.id = st.user_id
      LEFT JOIN users ua ON ua.id = st.updated_by
      LEFT JOIN projects pr ON pr.id = t.project_id
    ...
    const tasksQuery = `
      ...
      FROM tasks t
      LEFT JOIN users u ON u.id = t.user_id
      LEFT JOIN users ua ON ua.id = t.updated_by
      LEFT JOIN projects pr ON pr.id = t.project_id
      LEFT JOIN teams tm ON tm.id = u.team_id
      LEFT JOIN users u_assigned ON u_assigned.id = t.assigned_user_id
